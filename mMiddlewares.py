...
from db_worker import update_user_by_chat_id, update_user_by_message
...
class mLoggingMiddleware(BaseMiddleware):
    ...
    async def on_pre_process_error(self, update: types.Update, error, data: dict):
        ...
        for admin in config.admin_ids:
            try:
                await bot.send_message(admin, f"Process update [ID:{update.update_id}, NAME:{update.__class__.__name__}] failed with exception: {traceback.format_exc()}")
            except:
                pass
